...
/* Image URL Support + Working Product Edit + Notifications */
...
function saveNotifications() { localStorage.setItem('dissaNotifications', JSON.stringify(NOTIFICATIONS)); }
...
function updateShopInfoDisplay() {
  ...
  const hoursWeekEl = document.getElementById('hoursWeekday');
  const hoursSatEl = document.getElementById('hoursSat');
  const hoursSunEl = document.getElementById('hoursSun');
  ...
  if (hoursWeekEl) hoursWeekEl.textContent = shopInfo.openingHours.weekdays;
  if (hoursSatEl) hoursSatEl.textContent = shopInfo.openingHours.saturday;
  if (hoursSunEl) hoursSunEl.textContent = shopInfo.openingHours.sunday;
}
...
function renderProducts() {
  ...
  grid.innerHTML = shown.map(p => {
    ...
    const imageHtml = p.imageUrl && p.imageUrl.trim() !== '' 
      ? `<img src="${p.imageUrl}" alt="${p.name}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" /><span class="emoji-fallback" style="display:none">${p.emoji}</span>`
...
function updateStars() { 
  ...
  stars.forEach((s, i) => { s.style.opacity = i < currentRating ? '1' : '0.3'; }); 
}
function submitReview() {
  ...
  saveReviews(); renderReviews(); closeReviewForm(); 
  showToast('⭐ Thank you for your review!');
  addNotification(`⭐ New review from ${name} - ${currentRating} stars`);
}
  ...
  grid.innerHTML = RECIPES.map(r => 
    `<div class="recipe-card">
       ...
       <div class="recipe-body">
         ...
         <button onclick='addRecipeIngredients(${JSON.stringify(r.ingredients)})' style="margin-top:12px;width:100%;background:var(--green-pale);color:var(--green);border-radius:10px;padding:9px;cursor:pointer;">🛒 Add Ingredients</button>
       </div>
     </div>`
  ).join('');
}
...
function addRecipeIngredients(names) {
  ...
  names.forEach(name => { 
    ...
    if (p && p.stock > 0) { addToCart(p.id); added++; } 
  });
...
function showProdForm(id) {
  ...
  wrap.innerHTML = `<div class="prod-form">
    ...
    <div class="form-group">
      ...
      <small style="color:var(--text-light);display:block;margin-top:5px">
        💡 Use image hosting: Imgur (right-click → Copy Image Address), Google Drive, Cloudinary, or any direct image URL
      </small>
    </div>
  ...
  // Add image preview on URL change
...
function renderAnalytics() { 
  ...
  body.innerHTML = `<div class="stat-cards">
    ...
    <div class="stat-card"><div class="stat-card-val">${reviews.length}</div><div class="stat-card-label">Reviews</div></div>
